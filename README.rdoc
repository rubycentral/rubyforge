...
* The GForge user_group table gets mapped into the Redmine members table and member_roles table.
* The GForge artifact_group table gets mapped into rows in the Redmine projects_trackers table (projects HABTM trackers).  Redmine Trackers are not per Project; they are shared for the whole system.  The current migration only provisions rows in the projects_trackers table as they are needed - e.g., a Project won't get a "Bug" Tracker unless there's a GForgeArtifact in a "Bug" GForgeArtifactGroup in this project.  If there's a GForgeArtifactGroup with an unexpected name (e.g., "Tom's Issues") those get mapped into the "Bug" Tracker.
* The GForge artifact table gets mapped into the Redmine issues table.  GForge Artifacts have only 3 statuses (open/closed/deleted) whereas Redmine issues have 6 possible IssueStatus settings, so I just map them over to the IssueStatus with the same name.  GForge <tt>artifacts</tt> have 5 priority levels; these map onto Redmine's 5 standard <tt>IssueCategory</tt> values.
